...
var Context = classify({
  ...
  instanceMethods : {
    ...
    storeOperation : function(evolvedOperation, callback) {
      ...
      self._persistantOperation.save(evolvedOperation, function(err, savedOperation) {
        var howManyOps = null;

        if (err) {
          return callback(err);
        }
        howManyOps = savedOperation.getContext() + 1;
...
var i = 0; // TODO remove me (figure out why you needed me)
      ...
      self.checkHowManyOps(function(err, howManyOps) {
        ...
        async.whilst(
          function() { i++; console.log("about to check against howManyOps:\n", howManyOps); return (operation.context < howManyOps && i < 10); },
          function(cb) {
